...
*/
use Common\Extend\Payment\Wechatpay\SDKRuntimeException;
use Common\Extend\Payment\Wechatpay\Config;
...
class Common_util_pub extends Config
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$this->weChat_key;
...
{	
	...
	function getPrepayId()
	{
		$this->postXml();
		$this->result = $this->xmlToArray($this->response);
		if(strtolower($this->result['result_code']) == 'fail') {
    ...
    function getCode_url(){
        ...
        $this->result = $this->xmlToArray($this->response);
        if(strtolower($this->result['result_code']) == 'fail') {
            return array('error' => 1, 'msg' => $this->result['err_code_des']);
           // $this->error($this->result['err_code_des']);
        }
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $this->weChat_appId;//公众账号ID
		   	$this->returnParameters["mch_id"] = $this->weChat_mchId;//商户号
...
class JsApi_pub extends Common_util_pub
{
	 ...
	 * 	作用：生成可以获得code的url
	 */
	function createOauthUrlForCode($scope = "snsapi_base",$host = '')
	{
//		$host = empty($host) ? $_SERVER['HTTP_HOST'] : $host;
		$redirect_uri = $this->api_call_url;
		if(!empty($host)){
			if(strpos($this->api_call_url,'?')){
				$redirect_uri .= '&';
			}else{
				$redirect_uri .= '?';
			}
		...
		$urlObj["appid"] = $this->weChat_appId;
	...
	{
		$urlObj["appid"] = $this->weChat_appId;
		$urlObj["secret"] = $this->weChat_appSecret;
