	...
	export default {
		...
		methods: {
			getOrderDetail(payType) {
				// 商品列表
				let data = {
					rotate_id: this.$route.params.id,
					prize_id: this.$route.params.prize,
					address_id: this.address_id ? this.address_id : 0
				}
				this.$http.post('/rotate/goodsInfo.json' , data).then((res) => {
					res = res.json();
					if(res.status == 1){
						this.$nextTick(() => {
							this.$set('rotateList', res.data);
							// this.rotate_id = res.data.rotate_id;
			...
			addCodeCart(){
				...
				if(this.address_id == '' || this.address_id == null){
					this.pending = false;
					this.$dispatch('popup', '地址不能为空！');
				}else{
					this.$http.post('/rotate/buildOrder.json', data).then((res) => {
						res = res.json();
						if(res.status == 1){
	...
	<div class="container">
		...
		<dl class="order-item">
			...
			<dd>
				<ul class="goods-list">
					<li>
						<a class="goods-img" href="javascript:;" ><img v-for="item in rotateList.image" :src="item.img_url" alt="" /></a>
						<div class="goods-detail">
							<p><span class="fl">{{rotateList.goods_name}}</span><span>￥{{rotateList.shop_price}}</span></p>
							<p><del>￥{{rotateList.market_price}}</del></p>
							<p>x1</p>
						</div>
					</li>
				</ul>
		...
		<div class="order-item order-list">
			<div class="item">商品总计<span><p class="goods-points">{{rotateList.amount}}元</p></span></div>
			<div class="item">运费<span>￥{{rotateList.shipping_free}}元</span></div>
