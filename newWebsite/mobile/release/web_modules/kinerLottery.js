...
(function(win, doc, $) {
    ...
    KinerLottery.prototype.init = function() {
        ...
        var sn= $(this.opts.body).find('.KinerLotteryContent');

        sn.off('webkitTransitionEnd');
        sn.on('webkitTransitionEnd', function() {
            self.doing = false;
            var deg = $(self.opts.body).attr('data-deg');   
            // console.log($(self.opts.body).attr('data-deg'))    
