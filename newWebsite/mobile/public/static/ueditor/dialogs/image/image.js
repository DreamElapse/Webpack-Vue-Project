...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            //自主设置远程图片保存路径 (如果有版本更新请保留此段)
            var savePath = editor.getOpt('savePath');
            if(savePath) {
                actionUrl = actionUrl + '&savePath=' + savePath;
            }
            //自主设置结束
                ...
                switch (type) {
                    ...
                    case 'startUpload':
						...
						if(watermark == 1){
							...
							$('#watermark_position .selected').each(function(){
								watermark_position += ','+$(this).attr('value');
							});
							var watermark_file = $('#watermark_file').val();
							url = url + '&watermark=1&watermark_position='+watermark_position.substr(1,watermark_position.length-1)+'&watermark_file='+watermark_file;
						}
    ...
    OnlineImage.prototype = {
        ...
        /* 初始化第一次的数据 */
        initData: function (_getList_) {
			_getList_ = _getList_==false ? false : true;

            /* 拉取数据需要使用的值 */
            this.state = 0;
            this.listSize = editor.getOpt('imageManagerListSize');
            this.listIndex = 0;
	    this.dirlist = true;  //Add By Lemonice
            this.listEnd = false;

            /* 第一次拉取数据 */
            if(_getList_==true) this.getImageData(true);  //Add By Lemonice   第一次初始化获取文件夹
        },
        /* 重置界面 */
        reset: function() {
            this.initContainer();
        ...
        /* 向后台拉取图片列表数据 */
        getImageData: function (dir_list, dir_name) {
			dir_list = dir_list ? dir_list : false;  //Add By Lemonice
			dir_name = dir_name ? dir_name : '';  //Add By Lemonice
            var _this = this;
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                ajax.request(url, {
                    ...
                    'data': utils.extend({
							dir_list: (dir_list ? 1 : 0),  //Add By Lemonice
							dir_name: dir_name,  //Add By Lemonice
                    ...
                    'onsuccess': function (r) {
                        try {
                            ...
                            if (json.state == 'SUCCESS') {
								if(json.is_dir && json.is_dir == 1){
									_this.pushDirData(json.list);  //文件夹列表
									_this.dirlist = true;
								}else{
									 _this.pushData(json.list);  //图片列表
									 _this.dirlist = false;
		...
		/* 添加文件夹到列表界面上 Add By Lemonice */
        pushDirData: function (list) {
			...
			for (i = 0; i < list.length; i++) {
				if(list[i]){
					...
					img.width = 113;
					img.setAttribute('src', '/public/static/ueditor/dir_bg.jpg' );
					domUtils.addClass(icon, 'icon');
					
					icon.innerHTML = '<span style="background:#3c3c3c; color:#fff;">'+list[i]+'</span>';
					icon.setAttribute('_dir_name',list[i]);
					...
					icon.onclick = function(e){
						var dir_name = this.getAttribute('_dir_name');
						
						/*reset*/
						_this.initContainer();
            			_this.initData(false);
						_this.getImageData(false,dir_name);
					};
				}                
			}
        },
        ...
        pushData: function (list) {
			...
			for (i = 0; i < list.length; i++) {
				if(list[i] && list[i].url) {  
					...
					domUtils.addClass(icon, 'icon');
					icon.innerHTML = '<span style="background:#000; color:#fff;">'+list[i].width + " x " + list[i].height+'</span>';
