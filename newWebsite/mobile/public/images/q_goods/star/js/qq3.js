...
//					index=0;
	...
	getPageStr:function(){
		var str=(page.qzoneStr==0)?'page_a':'page_b';			
		return str;
	}
...
ves(function(){
	var viewModel={
		...
		qqchat:function(e){
			...
			var qq;			
			if(ves('html').hasClass('ann1')||ves('html').hasClass('mn1')){
				qq=e.current.qq1;
			}else if(window.location.host=='m.chinaskin.net'||ves('html').hasClass('mn2')||ves('html').hasClass('ann2')){
				qq=e.current.qq2;				
			}else if(ves('html').hasClass('mn3')||ves('html').hasClass('ann3')){
				qq=e.current.qq3;				
			}else if(ves('html').hasClass('mn')){
				qq=e.current.qq6;				
			}else if(ves('html').hasClass('Ann')){
				qq=e.current.qq8;				
			}else{
				qq=e.current.qq;				
			}
						...
						if(isClickAddTip){
							switch(this.getAttribute('class')){
								case 'qq':
									...
									break;
								case 'top_qq':
									page.statistic('QQ_top_'+name+'_sd');
									break;
								case 'bott_qq':
									page.statistic('QQ_bottom_'+name+'_sd');
									break;
								case 'poster':
									page.statistic('QQ_poster'+name+'_sd');
									break;
								case 'fixed_qq':
									page.statistic('QQ_Sus_'+name+'_'+page.getPageStr()+'_sd');
									break;
								case 'content_qq':
									page.statistic('QQ_txdb_'+name+'_'+page.getPageStr()+'_sd');
									break;
								default:
							...
							switch(this.getAttribute('class')){
								case 'qq':
									...
									break;
								case 'top_qq':
									page.statistic('QQ_top_'+name+'_zd');
									break;
								case 'bott_qq':
									page.statistic('QQ_bottom_'+name+'_zd');
									break;
								case 'poster':
									page.statistic('QQ_poster'+name+'_zd');
									break;
								case 'fixed_qq':
									page.statistic('QQ_Sus_'+name+'_'+page.getPageStr()+'_zd');
									break;
								case 'content_qq':
									page.statistic('QQ_txdb_'+name+'_'+page.getPageStr()+'_zd');
									break;
								default:
