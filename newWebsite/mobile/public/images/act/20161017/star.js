...
var input = document.querySelector('.star,.star1');
var field = {}
var hasFocus = false;
...
function reposition() {
	...
	if(navigator.userAgent.match(/Android|iPhone|SymbianOS|Windows Phone|iPad|iPod/)){
		...
		var inL = parseInt(input.getBoundingClientRect().left*2.6);
		var inT = 250;
		var inB = 700;
	}
	field = {
...
function burst(intensity) {
	...
	this.spray(intensity, function() {
		return [
			null, null,
			Vector.create(
				Random.between(rangeMin + 200, rangeMax - 100),
				Random.between(field.top + 150, field.bottom - 100)
			),
			Vector.random(force),
			size + Math.random(),
...
simulate(
	'2d', {
		...
		paint: function(particle) {
			...
			this.paint.circle(p.x, p.y, s, 'rgba(247,236,40,' + o + ')');
...
function simulate(dimensions, options) {
	...
	function go() {
		...
		window.onload = function(e) {
			...
			light_timer = setInterval(ooo,2000);
			// setTimeout(aaa,2000);
			var sw = false;
			window.onscroll = function(){

				var windowHeight = window.screen.height;
				if(scrollTop > windowHeight / 2 && !sw){
